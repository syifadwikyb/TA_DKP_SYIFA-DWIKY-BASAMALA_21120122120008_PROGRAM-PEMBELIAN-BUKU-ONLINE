...
class ButtonOke:
    ...
    def bayar_(self):
        bayar = self.pembayaran_objek.metode_var.get()
    
        if not bayar:
            messagebox.showinfo("Pemberitahuan", "Harap pilih metode pembayaran.")
        elif bayar == "Transfer Bank" or bayar == "Bayar di Kasir":
            messagebox.showinfo("Pemberitahuan", "Anda telah mencapai batas maksimal pembelian.")
class ButtonLagi:
    def __init__(self, input_frame, jumlah_objek, pilihan_objek, total_objek):
        self.input_frame = input_frame
